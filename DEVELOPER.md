...
"Run > Edit Configuration Menu" add create a `Node.js` configuration named "Karma
server".
...
Launch the server by selecting the "Karmer server" configuration in the toolbar and
pressing the play icon. You should see the following message at the bootom of the 
run window:
...
To excecute the test suite, you just need to run this "Karma tests" configuration. 
You should make sure to have executed "Karma server" first.

### Debugging

You need to create a "JavaScipt Debug" configuration named "Karma debug". Set 
