...
void testWithGetterFactory(FieldGetterFactory getterFactory) {
  describe('DirtyCheckingChangeDetector with ${getterFactory.runtimeType}', () {
    ...
    describe('object field', () {
      ...
      it('should detect observable field changes', async(() {
    ...
    describe('insertions / removals', () {
      ...
      it('should cancel notification subscription on group removal', async(() {
        var obj = {};
      ...
      it('should cancel notification subscription on record removal', async(() {
        var observable = new _ObservableUser();
        expect(observable.hasObservers).toEqual(false);
    ...
    describe('list watching', () {
      ...
      addListSpec(useObservable: false);
      addListSpec(useObservable: true);
    });
...
void addListSpec({bool useObservable}) {
  ...
  Iterator<Record> getChangeIterator() {
    ...
    return detector.collectChanges();
  }

  describe('use observable: $useObservable', () {
    describe('previous state', () {
      it('should store on addition', wrap(() {
      ...
      xit('should handle swapping elements correctly', wrap(() {
        var list = listFactory([1, 2]);
        print(list.runtimeType);
        detector.watch(list, null, null);
        getChangeIterator().moveNext();
        var iterator;
      ...
      it('should handle swapping elements correctly - gh1097', wrap(() {
        // This test would only have failed in non-checked mode only
        var list = listFactory(['a', 'b', 'c']);
        var record = detector.watch(list, null, null);
    ...
    it('should detect changes in list', wrap(() {
      var list = listFactory([]);
      var record = detector.watch(list, null, 'handler');
      expect(getChangeIterator().moveNext()).toEqual(false);
      var iterator;
    ...
    it('should test string by value rather than by reference', wrap(() {
      var list = listFactory(['a', 'boo']);
      detector.watch(list, null, null);
      getChangeIterator();
    ...
    it('should ignore [NaN] != [NaN]', wrap(() {
      var list = listFactory([double.NAN]);
      var record = detector.watch(list, null, null);
      getChangeIterator();
    ...
    it('should remove and add same item', wrap(() {
      var list = listFactory(['a', 'b', 'c']);
      var record = detector.watch(list, null, 'handler');
      var iterator;
      getChangeIterator();
    ...
    it('should support duplicates', wrap(() {
      var list = listFactory(['a', 'a', 'a', 'b', 'b']);
      var record = detector.watch(list, null, 'handler');
      getChangeIterator();
    ...
    it('should properly support objects with equality', wrap(() {
      FooBar.fooIds = 0;
      var list = listFactory([new FooBar('a', 'a'), new FooBar('a', 'a')]);
      var record = detector.watch(list, null, 'handler');
      var iterator;
    ...
    it('should not report unnecessary moves', wrap(() {
      var list = listFactory(['a', 'b', 'c']);
      var record = detector.watch(list, null, null);
      var iterator = detector.collectChanges()..moveNext();
...
void addMapSpec({bool useObservable}) {
  Function wrap(fn) => useObservable ? async(fn) : sync(fn);

  Map mapFactory(Map map) => useObservable ? new ObservableMap.from(map) : map;
  ...
  describe('use observable: $useObservable', () {
    describe('previous state', () {
      ...
      it('should handle changing key/values correctly', wrap(() {
        var map = mapFactory({1: 10, 2: 20});
        var record = detector.watch(map, null, null);
        getChangeIterator().moveNext();
        var iterator;
    ...
    it('should do basic map watching', wrap(() {
      var map = mapFactory({});
    ...
    it('should test string keys by value rather than by reference', wrap(() {
      var map = mapFactory({'foo': 0});
    ...
    it('should test string values by value rather than by reference', wrap(() {
    ...
    it('should not see a NaN value as a change', wrap(() {
      var map = mapFactory({'foo': double.NAN});
      detector.watch(map, null, null);
      getChangeIterator();
...
class _ObservableUser extends ChangeNotifier {
  ...
  void set first(String value) {
    __$first = notifyPropertyChange(#first, __$first, value);
  }

  String __$last;
  String get last => __$last;
  void set last(String value) {
    __$last = notifyPropertyChange(#first, __$last, value);
  }

  _ObservableUser([String first, String last]) {
    this.first = first;
    this.last = last;
  }
}
