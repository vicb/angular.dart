  ...
  FactoryFn call(dom.Element element) {
    return (Injector injector) {
            ...
            if (_viewFuture != null) {
              return _viewFuture.then((ViewFactory viewFactory) {
                if (shadowScope.isAttached) {
                  shadowDom.nodes.addAll(viewFactory(shadowInjector).nodes);
                }
                return shadowDom;
              });
            }
      ...
      var shadowModule = new Module()
          ...
          ..bindByKey(SCOPE_KEY, toFactory: (Injector i) =>
              scope.createChild(i.getByKey(_ref.typeKey)))
      ...
      if (_f.config.elementProbeEnabled) {
        probe = _f.expando[shadowDom] = new ElementProbe(
